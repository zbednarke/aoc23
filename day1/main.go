...
func main() {
	// we are given a map, which has stars on 50 locations with highest probability of having problems
	// i have to check all fifty star locaitons by 12/25.  today is 12/1

	// collect stars by solving puzzles.  two puzzles each day.  second puzzle unlocked after first is solved.

	// they are loading me into a trebuchet
	// their calibration document, my puzzle input, has been amended in a way obscuring the values.

	// this document constists of liines of text; each line originally had a specific calibration value that is encrypted.
	// on each line, the value is a combination of the first digit and the last digit to form a single 2 digit number.

	// puzzle output is the sum of all these numbers.

	// SOlution:
	// for each line, strip out all non numberical characters.  treat the remaining number(s) as an array. then take the first and last elements of the array to form a 2 digit numbers.  and sum them all up.
	...
	for _, value := range input {
		numbers := removeNonNumeric(value)
		firstInt, err := strconv.Atoi(string(numbers[0]))
		if err != nil {
			fmt.Println(err)
		}
		secondInt, err := strconv.Atoi(string(numbers[len(numbers)-1]))
		if err != nil {
			fmt.Println(err)
		}
		this := firstInt*10 + secondInt
		sum += this
	}
	...
	for i, value := range input {
		digitInfos := findAllDigits(value)

		n1 := digitInfos[0].number
		lastN := digitInfos[len(digitInfos)-1].number

		fmt.Println(i, value)
		fmt.Println(digitInfos)
		fmt.Printf("%d%d\n", n1, lastN)
...
type digitInfo struct {
	number    int
	digitType string // "word" or "char"
	position  int
}

func findAllDigits(s string) []digitInfo {
	var allDigits []digitInfo

	// Finding digit characters
	for i, r := range s {
		if r >= '0' && r <= '9' {
			numberInt, err := strconv.Atoi(string(r))
			if err != nil {
				fmt.Println(err)
			}
			allDigits = append(allDigits, digitInfo{number: numberInt, digitType: "char", position: i})
		}
	}

	// Finding English digit words
	digitWords := []string{"zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"}
	for number, word := range digitWords {
		re := regexp.MustCompile(word)
		matches := re.FindAllStringIndex(s, -1)
		for _, match := range matches {
			allDigits = append(allDigits, digitInfo{number: number, digitType: "word", position: match[0]})
		}
	}

	// Sort all digits by their positions
	sort.Slice(allDigits, func(i, j int) bool {
		return allDigits[i].position < allDigits[j].position
	})
